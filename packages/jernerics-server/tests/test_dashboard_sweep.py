...
class TestSubNavAvailability:
    def test_via_scopes_series_points_and_search(self, service, investigation_id):
        ...
        for view in ("series", "points"):
            doc = decode_view_state(unquote(hrefs[view.title()].split("view=")[1]))
            assert doc["inv"]["view"] == view
            assert doc["inv"]["member"] == str(SWEEP)
        search_doc = decode_view_state(unquote(hrefs["Search"].split("view=")[1]))
