"""Tests for the retry_checker using make_adapter."""
...
class TestRetryCheckerUsesAdapter:
    ...
    @patch("jernerics.retry_checker.load_config")
    @patch("jernerics.retry_checker.load_backend_config")
    def test_calls_make_adapter_not_make_backend(
        self, mock_load_backend, mock_load_config, mock_make_adapter, tmp_path
    ):
        from jernerics.retry_checker import run_checker
        ...
        # Verify: called make_adapter, not make_backend
        mock_make_adapter.assert_called_once()
        call_kwargs = mock_make_adapter.call_args
        assert call_kwargs[1]["host"].__class__.__name__ == "StdoutHost"

        # Verify: adapter.submit_sweep called with SweepSubmissionParams
    ...
    @patch("jernerics.retry_checker.load_config")
    @patch("jernerics.retry_checker.load_backend_config")
    def test_passes_overrides_to_params(
        self, mock_load_backend, mock_load_config, mock_make_adapter, tmp_path
    ):
        from jernerics.retry_checker import run_checker
        ...
        slurm_config = SlurmConfig(partition="batch", time="2:00:00")
        shared = SharedConfig(
            name="hpc",
            type="slurm",
            remote_dir="/scratch/proj",
        ...
        backend_config = MagicMock()
        backend_config.shared = shared
        backend_config.backend = slurm_config
        backend_config.container = MagicMock()
        mock_load_backend.return_value = backend_config
        ...
        sweep.n_trials = 6
        sweep.direction = "minimize"
        sweep.backend_overrides = {"hpc": {"time": "4:00:00"}}
        sweep.sampler = None
        mock_load_config.return_value = sweep
        ...
        assert params.overrides["mem"] == "64G"
