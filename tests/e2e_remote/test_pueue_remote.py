...
def test_basic(scimlab_server):
    ...
    assert wait_for_trial_end(server, study, 5, timeout=300), (
...
def test_retry(scimlab_server):
    ...
    settled = wait_for_settled(server, study, min_count=4, timeout=300)
...
def test_gpu(scimlab_server):
    ...
    assert wait_for_trial_end(server, study, 1, timeout=300), (
